...
path = '/home/luiz/Jupyter_Notebook/Python_Notebook/Climate_Extremes_DOC/Scripts_Extremes/PDF-Precip-SA/'
...
# Load datasets
df = pd.read_excel('~/Jupyter_Notebook/Python_Notebook/Climate_Extremes_DOC/mean_precip-SA-PDF.xlsx')
...
for index, unit, abc, ax in zip(list_index, list_units, ABC, enumerate(axs)):
    for period, color in zip(list_periods, colors):
        # Plot pdf
        filter_data = df.query('Index == @index & Period == @period')['Mean']
    ...
    # Filter data mean
    filter_data_mean = df.query('Index == @index')['Mean'].mean()
    # Plot vertical line
...
# Save figure
fig.savefig(f'{path}SA_Precip.png', bbox_inches='tight', dpi=350)
# plt.show()
